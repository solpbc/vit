...
describe('vit remix', () => {
  ...
  test('fails with no arguments', () => {
    const result = run('remix', '/tmp', { CLAUDECODE: '1' });
    expect(result.exitCode).not.toBe(0);
  });

  test('rejects invalid ref format', () => {
    const result = run('remix not-valid', '/tmp', { CLAUDECODE: '1' });
    expect(result.exitCode).not.toBe(0);
  ...
  test('errors when no DID configured', () => {
    ...
    const result = run('remix fast-cache-invalidation', '/tmp', { CLAUDECODE: '1', XDG_CONFIG_HOME: configHome });
    expect(result.exitCode).not.toBe(0);
  ...
  test('errors when no beacon is set', () => {
    const result = run('remix fast-cache-invalidation --did did:plc:test123', '/tmp', { CLAUDECODE: '1' });
    expect(result.exitCode).not.toBe(0);
